...
train_df <- X_matrix[train_index,]
test_df <- X_matrix[test_index,]
train_label <- Y_label[train_index]
test_label <- Y_label[test_index]
...
order(importance(train_rf)[51:100])

#Fit and visualize random forest:
library(xgboost)
d_train <- xgb.DMatrix(data = train_df, label = as.matrix(train_label))
d_test <- xgb.DMatrix(data = test_df, label = as.matrix(test_label))
params <- list(booster = "gbtree", objective = "reg:linear", 
               eta=0.3, gamma=0, max_depth=6, min_child_weight=1, 
               subsample=1, colsample_bytree=1)
xgb1 <- xgb.train(params = params, data = d_train, watchlist = list(val = d_test, train = d_train), nrounds = 200, early_stopping_rounds = 15,
                  print_every_n = 10)
xgb.importance(colnames(d_train), model = xgb1)
